...
FINDPERMS_PERMS_DIRS_CANDIDATES="${FINDPERMS_PERMS_DIRS_CANDIDATES:-"public private data"}"
...
configure() {
    ...
    for i in $(find etc -name "*.envsubst" -type f 2>/dev/null);do
        di="/$(dirname $i)" \
            ...
            && cp "$i" "/$i" \
            && CONF_PREFIX="$FLASK_CONF_PREFIX" confenvsubst.sh "/$i" \
            && rm -f "/$i"
    done
...
do_fg() {
    ( cd $PROJECT_DIR \
        && FLASK_APP="$FLASK_APP" exec gosu $APP_USER \
