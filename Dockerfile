...
RUN bash -c 'set -ex \
    ...
    && if ! ( getent passwd flask &>/dev/null );then useradd -ms /bin/bash flask --uid 1000;fi && date'
...
ADD --chown=flask:flask local/*/sys/add_*sh /code/sys/
ARG WITH_IMPOSM=
...
ADD --chown=flask:flask sys                          /code/sys
ADD --chown=flask:flask local/flask-deploy-common/  /code/local/flask-deploy-common/
...
ADD --chown=flask:flask .git                         /code/.git
